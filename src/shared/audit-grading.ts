...
export interface AuditGradeInfo {
...
export function calculateAuditGrade(params: {
  ...
  const criticalPenalty = Math.min(60, criticalCount * (pagesCrawled === 1 ? 22 : Math.max(5, Math.round(30 / Math.sqrt(pagesCrawled)))));
  const warningPenalty = Math.min(30, warningCount * (pagesCrawled === 1 ? 8 : Math.max(2, Math.round(15 / Math.sqrt(pagesCrawled)))));
  ...
  if (params.lighthouseScores && params.lighthouseScores.length > 0) {
    const validScores = params.lighthouseScores.filter((s): s is number => s != null && !isNaN(s));
    if (validScores.length > 0) {
      const avgLh = validScores.reduce((acc, curr) => acc + curr, 0) / validScores.length;
  ...
  let badgeClass = "bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20";
  let textClass = "text-rose-600 dark:text-rose-400";
  let bgClass = "bg-rose-500";
  let borderClass = "border-rose-500";
  let summary = "Severe technical defects detected. Immediate crawler optimization required.";
  ...
  if (score >= 70) {
    ...
    if (score >= 90) {
      ...
      summary = "Website exhibits excellent technical search compliance with minimal or zero blockers.";
    } else if (score >= 80) {
      letterGrade = "A";
      ...
      summary = "Solid baseline health with a few manageable optimization opportunities.";
    }
  } else if (score >= 50) {
    category = "orange";
    ...
    if (score >= 60) {
      ...
      summary = "Noticeable technical issues impacting crawl efficiency and search indexing.";
    } else {
      letterGrade = "D";
