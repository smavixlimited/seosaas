...
export const ConversionAdReadinessService = {
  ...
  async scanTrackingPixels(targetUrl: string): Promise<{
    ...
    try {
      const resp = await fetch(targetUrl, {
        headers: { "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) SkorviaAdAudit/1.0" },
    ...
    const hasMeta = lower.includes("connect.facebook.net") || lower.includes("fbevents.js") || lower.includes("fbq(");
    const metaIdMatch = html.match(/fbq\s*\(\s*['"]init['"]\s*,\s*['"](\d+)['"]/i);
    pixels.push({
      id: "pixel_meta",
      name: "Meta Pixel (Facebook & Instagram)",
      type: "meta",
      pixelId: metaIdMatch ? metaIdMatch[1] : (hasMeta ? "Active (Injected)" : null),
      status: hasMeta ? "active" : "missing",
      eventsDetected: hasMeta ? (lower.includes("purchase") ? ["PageView", "Purchase"] : ["PageView"]) : [],
      details: hasMeta
    ...
    const hasGAds = lower.includes("googleads") || lower.includes("conversion_id") || /aw-\d+/i.test(html);
    ...
    const hasLinkedIn = lower.includes("snap.licdn.com") || lower.includes("_linkedin_partner_id");
    pixels.push({
      id: "pixel_linkedin",
    ...
    const hasGa4 = lower.includes("google-analytics.com") || /G-[A-Z0-9]+/i.test(html);
   ...
   */
  async getConversionAudit(projectId: string, targetUrl: string, domain?: string): Promise<ConversionAuditResult> {
    ...
    try {
      ...
      if (existing) {
        ...
        if (age < CACHE_TTL_MS) {
          const detectedPixels: DetectedTrackingPixel[] = existing.detectedPixelsJson
          ...
          return {
            ...
            hasAdPixelInstalled: detectedPixels.some((p) => p.status === "active" && (p.type === "meta" || p.type === "google_ads")),
            detectedPixels,
            checksPassed: JSON.parse(existing.checksPassedJson || "[]"),
            criticalFrictionPoints: JSON.parse(existing.criticalFrictionPointsJson || "[]"),
   ...
   */
  async runConversionAudit(projectId: string, targetUrl: string, domain?: string): Promise<ConversionAuditResult> {
    const cleanUrl = targetUrl.trim();
    const cleanDomain = domain || cleanUrl.replace(/^https?:\/\//, "").replace(/\/.*$/, "");
    ...
    let adWastedSpendRisk: AdSpendRisk = pixelScan.hasAdPixelInstalled ? "low" : "critical";
    ...
    let checksPassed: ConversionCheckItem[] = [
      {
        ...
        detail: "Site enforces HTTPS and secure cookie policies across checkout endpoints.",
      },
      {
        category: "Trust & Security",
        label: "Visible Privacy Policy & Terms of Service",
        detail: "Legal footer links are accessible on landing pages, complying with Google Ads policies.",
      },
      {
        category: "Offer Clarity",
        label: "Above-the-Fold Primary Call-to-Action",
        detail: "Hero section contains a distinct primary action button without competing secondary links.",
      },
      {
        category: "Social Proof",
        label: "Customer Rating Badge & Review Snippets",
        detail: "Identified verified customer review quotes and 5-star rating badges.",
      },
      {
        category: "Frictionless Onboarding",
        label: "Frictionless Single-Step Sign-up",
        detail: "Checkout form requires minimal initial fields, reducing checkout abandonment.",
      },
    ];
    ...
    if (pixelScan.hasAdPixelInstalled) {
      checksPassed.push({
        ...
        detail: "Meta Pixel / Google Ads conversion tracking is active and receiving events.",
      });
    }
    ...
    if (!pixelScan.hasAdPixelInstalled) {
      criticalFrictionPoints.push({
        ...
        issue: "Driving paid traffic to this page without a conversion pixel prevents Smart Bidding (pMax / Advantage+) from optimizing for purchases or leads. You cannot track true ROAS or build custom retargeting audiences.",
    ...
    criticalFrictionPoints.push(
      {
        ...
        issue: "Paid traffic users have higher purchase hesitation. Lack of a prominent '14-Day Money-Back Guarantee' or 'No Credit Card Required' badge directly lowers ad ROAS.",
        impact: "-15% paid ad conversion velocity",
      },
      {
        severity: "advisory",
        title: "Mobile Largest Contentful Paint (LCP) Above 2.2s",
        issue: "Paid mobile clicks bounce rapidly if the above-fold hero banner takes longer than 2.0s to render.",
    ...
    if (!pixelScan.hasAdPixelInstalled) {
      recommendedFixes.push({
        id: "fix_pixel",
        priority: "HIGH",
        title: "Install Meta Pixel & Google Ads Tag Before Launching Paid Traffic",
        action: "Embed Meta Pixel base script and Google Ads conversion tag on your landing page. Set up standard conversion events for 'Lead' or 'Purchase' to enable smart bidding.",
    ...
    recommendedFixes.push(
      {
        ...
        action: "Embed '14-Day Money-Back Guarantee', 'Cancel Anytime', and 'Instant Setup' badges immediately below the primary signup button.",
      ...
      },
      {
        ...
        action: "Rewrite the sub-headline to state the exact quantitative outcome (e.g. 'Boost organic traffic by 40% in 60 days without hiring an agency') rather than generic feature descriptions.",
      ...
      },
      {
        ...
        action: "Add 1-click Google OAuth button alongside standard email/password registration to eliminate mobile typing friction.",
...
JSON format:
{
  ...
  "frictionAndFormLengthScore": number (0-100)
      ...
      if (parsed.overallScore) {
        overallScore = pixelScan.hasAdPixelInstalled ? parsed.overallScore : Math.min(68, parsed.overallScore);
        grade = parsed.grade || grade;
        adWastedSpendRisk = pixelScan.hasAdPixelInstalled ? (parsed.adWastedSpendRisk || adWastedSpendRisk) : "critical";
        trustAndCredibilityScore = parsed.trustAndCredibilityScore || trustAndCredibilityScore;
        ctaAndOfferClarityScore = parsed.ctaAndOfferClarityScore || ctaAndOfferClarityScore;
        pageSpeedAndMobileScore = parsed.pageSpeedAndMobileScore || pageSpeedAndMobileScore;
        socialProofAndReviewsScore = parsed.socialProofAndReviewsScore || socialProofAndReviewsScore;
        frictionAndFormLengthScore = parsed.frictionAndFormLengthScore || frictionAndFormLengthScore;
      }
    } catch (aiErr) {
    ...
    try {
      ...
      await db
        .insert(conversionAdReadinessAudits)
        .values({
          id,
          projectId,
          targetUrl: cleanUrl,
