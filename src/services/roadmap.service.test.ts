...
describe("RoadmapService (Actionable Growth Engine & 3-Way Verification)", () => {
  it("seeds a rich, prioritized SEO sprint when project roadmap is empty", async () => {
    const tasks = await RoadmapService.seedInitialRoadmap("test-proj-777", "example-saas.com");
    ...
    tasks.forEach((task) => {
      ...
      expect(["todo", "in_progress", "completed", "dismissed"]).toContain(task.status);
    });
  });

  it("calculates accurate roadmap metrics and health score boost", async () => {
    const metrics = await RoadmapService.getRoadmapMetrics("test-proj-777", "example-saas.com");
  ...
  it("creates custom growth roadmap tasks", async () => {
    const custom = await RoadmapService.createCustomTask("test-proj-777", {
      ...
      description: "Notify growth team on first of the month with ranking shifts.",
      category: "growth",
      priority: "high",
      estimatedMinutes: 10,
    });
    ...
    expect(custom.title).toBe("Set up GSC automated monthly audit email digest");
