...
describe("BlogCmsService (Blog CMS & SEO Publishing Engine)", () => {
  ...
  describe("Utility Helpers", () => {
    it("generates clean URL slugs from article titles", () => {
      const slug = BlogCmsService.generateSlug("10 Proven Ways to Rank #1 on Perplexity & Google!");
  ...
  describe("Post Retrieval & Filters", () => {
    ...
    it("retrieves a post by slug", async () => {
      const post = await BlogCmsService.getBlogPostBySlug("guide-to-ai-engine-optimization-aeo");
  ...
  describe("Post CRUD Operations", () => {
    it("creates and updates a blog post", async () => {
      const created = await BlogCmsService.upsertBlogPost(
        {
          ...
          description: "An in-depth forecast of AI search trends and semantic ranking models.",
      ...
      expect(created.canonicalUrl).toContain("/blogs/next-gen-saas-seo-trends-for-2027");
