...
// Adapt one Skorvia tool into an AI SDK tool. The shared handler receives the
 ...
 * free site-reading tools. Every tool the Skorvia MCP server exposes is
...
export function buildSamMcpTools(
  ...
  return {
    ...
    get_product_info: tool({
      description:
        "The Skorvia fact sheet: what the product does, plans/pricing, credit costs, integrations, MCP setup. Call before answering questions about Skorvia itself. Uses no credits.",
      inputSchema: z.object({}),
    ...
    search_live_web: tool({
      description:
        "Search the live web and extract clean Markdown from the top search results in real time using Firecrawl.",
      inputSchema: z.object({
