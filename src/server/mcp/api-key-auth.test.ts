...
function request(headers?: HeadersInit, method = "POST") {
  return new Request("https://app.skorvia.com/mcp", { method, headers });
}
...
describe("handleMcpApiKeyRequest", () => {
  ...
  it("leaves non-Skorvia bearer tokens for OAuth", async () => {
    await expect(
  ...
  it("leaves non-Skorvia x-api-key values for OAuth", async () => {
    await expect(
      handleMcpApiKeyRequest(
        request({
