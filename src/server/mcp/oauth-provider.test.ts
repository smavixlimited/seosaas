...
vi.mock("@/lib/auth", () => ({
  getHostedBaseUrl: () => "https://app.skorvia.com",
}));
...
describe("Skorvia OAuth provider configuration", () => {
  beforeEach(() => {
  ...
  it("binds tokens and protected-resource metadata to the canonical MCP URL", async () => {
    ...
    await dispatch(provider, new Request("https://app.skorvia.com/health"));

    expect(mocks.options).toHaveLength(1);
    expect(mocks.options[0]?.resourceMetadata).toEqual({
      resource: "https://app.skorvia.com/mcp",
      scopes_supported: ["mcp"],
      resource_name: "Skorvia MCP",
    });
  ...
  it("lets the provider issue Perplexity a real client secret", async () => {
    ...
    await dispatch(
      provider,
      new Request("https://app.skorvia.com/api/auth/oauth2/register", {
        method: "POST",
  ...
  it("includes the authorization-server issuer when consent is denied", async () => {
    ...
    const response = await invokeDefaultHandler(
      new Request("https://app.skorvia.com/api/oauth/consent", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Origin: "https://app.skorvia.com",
        },
  ...
  it("redirects safe authorization errors with state and issuer", async () => {
    ...
    const response = await invokeDefaultHandler(
      new Request("https://app.skorvia.com/api/auth/oauth2/authorize"),
      {
        OAUTH_PROVIDER: {
          parseAuthRequest: () =>
            Promise.reject(
