 ...
 * We support all 240+ world countries and territories mapped from LOCATION_OPTIONS.
 */
export const WEB_SEARCH_COUNTRY_CODES = Array.from(
  new Set(
    LOCATION_OPTIONS.map((opt) =>
      (opt.shortLabel === "UK" ? "GB" : opt.shortLabel).toUpperCase(),
    ),
  ),
);

export const webSearchCountryCodeSchema = z
  .string()
  .min(2)
  .max(2)
  .transform((code) => code.toUpperCase());

export type WebSearchCountryCode = string;
