...
import { getInvitationByTokenServerFn, acceptTeamInviteServerFn } from "@/serverFunctions/team-management";
...
function AcceptInvitePage() {
  ...
  const acceptMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Welcome to the team! You now have access to the workspace.");
      ...
      <div className="w-full max-w-md bg-base-100 rounded-3xl border border-base-300 shadow-xl p-8 space-y-6 text-center">
        ...
        <div className="space-y-2">
          <div className="w-14 h-14 mx-auto rounded-2xl bg-primary/10 text-primary flex items-center justify-center">
            <Icon icon="solar:users-group-two-rounded-bold-duotone" className="h-8 w-8" />
          </div>
          <h1 className="text-2xl font-black tracking-tight text-base-content">
            Team Workspace Invitation
          </h1>
          <p className="text-xs text-base-content/60">
            Collaborate on SEO campaigns, keyword intelligence, and site audits on {BRAND_CONFIG.name}.
          </p>
        </div>
        ...
        {(!inviteQuery.isLoading && (!invite || isExpired || invite.status !== "pending")) && (
          <div className="p-5 rounded-2xl bg-error/10 border border-error/20 text-error text-xs space-y-3">
            <Icon icon="solar:danger-triangle-bold" className="h-6 w-6 mx-auto" />
            ...
            <Link to="/sign-in" className="btn btn-sm btn-ghost text-primary font-bold">
          ...
          <div className="space-y-6">
            <div className="p-4 rounded-2xl bg-base-200/50 border border-base-300 text-left text-xs space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-base-content">{invite.email}</span>
              </div>
              <div className="flex items-center justify-between">
            ...
            {session?.user ? (
              ...
              >
                ...
                <span>{acceptMutation.isPending ? "Joining Workspace..." : "Accept Invitation & Join Team"}</span>
              </button>
            ) : (
        ...
        <div className="border-t border-base-200 pt-4 text-[11px] text-base-content/40">
          Powered by <strong className="text-primary">{BRAND_CONFIG.name}</strong> Enterprise SEO
        </div>
      </div>
    </div>
  );
