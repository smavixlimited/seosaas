...
function SavedKeywordsPage() {
  ...
  const importMutation = useMutation({
    mutationFn: async (items: ParsedMigrationKeyword[]) => {
      const keywords = items.map((i) => i.keyword);
      const metrics = items.map((i) => ({
        keyword: i.keyword,
        searchVolume: i.volume,
        cpc: i.cpc,
        keywordDifficulty: i.difficulty,
        competition: i.difficulty != null ? i.difficulty / 100 : undefined,
      }));
      await saveKeywords({
        data: {
          projectId,
  ...
  const isCompletelyEmpty = !isLoading && totalCount === 0 && !hasActiveFilters;
    ...
    <div className="overflow-auto px-4 py-4 pb-24 md:px-6 md:py-6 md:pb-8">
      <div className="mx-auto max-w-6xl space-y-4">
        <SavedKeywordsHeader
          ...
          onOpenCsvImporter={() => setShowCsvModal(true)}
        />

        {isCompletelyEmpty ? (
          <ProjectQuickStartEmptyState
            projectId={projectId}
            onOpenCsvImporter={() => setShowCsvModal(true)}
          />
        ) : (
        ...
        <CsvMigrationImporterModal
          ...
          onImport={(items) => importMutation.mutateAsync(items)}
        />
      </div>
    </div>
  );
