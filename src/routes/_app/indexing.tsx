...
import { VenixTable, VenixStatusBadge } from "@/client/components/table/VenixTable";
...
function IndexingStudioPage() {
  ...
  const [activeTab, setActiveTab] = React.useState<"manual" | "sitemap">("manual");
  ...
  const sitemapMutation = useMutation({
    mutationFn: (sUrl: string) => fetchSitemapForIndexing({ data: { sitemapUrl: sUrl } }),
    onSuccess: (res) => {
      toast.success(`Discovered ${res.urlCount} URLs from sitemap! Populated in batch editor.`);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="badge badge-primary badge-sm font-bold text-xs">AEO & IndexNow Fast-Track</span>
            <span className="text-xs text-base-content/50">Multi-Search Engine Distribution</span>
          </div>
          <h1 className="text-2xl font-black tracking-tight text-base-content mt-1">
            Instant Indexing Studio
          </h1>
          <p className="text-xs text-base-content/60">
            Submit new or updated URLs directly to Microsoft Bing, Yandex, Seznam, and Naver with zero crawl lag.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm">
          <div className="flex items-center justify-between text-xs font-bold text-base-content/60">
            ...
            <Icon icon="solar:global-bold-duotone" className="h-5 w-5 text-primary" />
          </div>
          <div className="text-xl font-black text-base-content mt-2">4 Search Engines</div>
          <div className="text-[11px] text-base-content/60 mt-1">Bing, Yandex, Seznam, Naver</div>
        </div>

        <div className="rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm">
          <div className="flex items-center justify-between text-xs font-bold text-base-content/60">
            <span>Total Submissions</span>
            <Icon icon="solar:document-text-bold-duotone" className="h-5 w-5 text-blue-500" />
          </div>
          <div className="text-xl font-black text-base-content mt-2">
            {history.reduce((acc, curr) => acc + curr.urlCount, 0).toLocaleString()} URLs
          </div>
        ...
        <div className="rounded-2xl border border-base-300 bg-base-100 p-4 shadow-sm">
          <div className="flex items-center justify-between text-xs font-bold text-base-content/60">
            ...
            <Icon icon="solar:clock-circle-bold-duotone" className="h-5 w-5 text-emerald-500" />
          </div>
          <div className="text-xl font-black text-base-content mt-2">&lt; 15 Minutes</div>
          <div className="text-[11px] text-base-content/60 mt-1">Immediate crawler dispatch</div>
        </div>
      </div>
          ...
          {activeTab === "manual" ? (
            <form onSubmit={handleManualSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-bold text-base-content/70">Host Domain</label>
                <input
                  type="text"
                  required
              ...
              <div className="flex items-center justify-between pt-2">
                <span className="text-xs text-base-content/50 font-medium">
                  {urlsInput.split("\n").filter((l) => l.trim().length > 0).length} URLs queued for push
                </span>
                ...
                >
                  ...
                  <span>{submitMutation.isPending ? "Indexing Now..." : "Index Now"}</span>
                </button>
              </div>
            </form>
          ) : (
            <form onSubmit={handleSitemapFetch} className="space-y-4 max-w-xl">
              <div className="space-y-1">
                <label className="text-xs font-bold text-base-content/70">Sitemap XML URL</label>
                <input
                  type="text"
                  required
              ...
              <p className="text-xs text-base-content/60 leading-relaxed">
                Skorvia will automatically crawl your XML sitemap, extract every valid canonical URL, and load them into the batch submission editor.
              </p>
              ...
              <button
                ...
                className="btn btn-primary rounded-2xl px-6 font-bold text-white shadow-md shadow-primary/20 gap-2"
              >
                <Icon icon="solar:cloud-download-bold-duotone" className="h-4 w-4" />
                <span>{sitemapMutation.isPending ? "Parsing Sitemap XML..." : "Fetch & Stage Sitemap URLs"}</span>
              </button>
            </form>
          )}
        </div>
      ...
      >
        ...
        <tbody>
          {history.length === 0 ? (
            <tr>
              <td colSpan={5} className="py-8 text-center text-base-content/40 font-medium">
            ...
            history.map((item) => (
              <tr key={item.id} className="hover:bg-base-200/50 transition-colors">
                ...
                <td className="font-mono font-bold text-primary">{item.urlCount} URLs</td>
                <td>
                  <VenixStatusBadge
                    status={item.statusCode >= 200 && item.statusCode < 300 ? "success" : "danger"}
