...
Two commands in Claude Code install the Skorvia MCP server and all nine SEO agent skills:

```
/plugin marketplace add every-app/skorvia
...
That works with or without Skorvia connected. With it, the answers come with real numbers attached.
...
Your time, plus data. The data runs $10 a month with [Skorvia](/pricing), $100 and up with the big suites, or a $50 minimum deposit with a raw data provider if you [self-host](/open-source-seo). Content costs whatever your time is worth. You don't need an agency to start.
