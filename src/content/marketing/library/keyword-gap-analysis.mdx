...
## How to run a keyword gap in Skorvia

Skorvia does not have a one-click "keyword gap" report that takes two domains and returns the difference. What it has are the two halves: you pull ranked keywords for each domain and the agent performs the join. In practice this is more flexible than a fixed report, because you control the filters on both sides, but it is worth knowing before you go looking for a button.
...
```text
Using the Skorvia MCP, run a keyword gap between [mydomain.com]
and [competitor.com].
