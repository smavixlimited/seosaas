...
![A ranked-keyword list filtered to positions 10 through 13, showing the keyword, its position, and monthly search volume for each row](/library/keyword-research/gsc-striking-distance-openseo.png)
...
Because the Search Console connection runs through Skorvia, your agent can query it alongside your ranking data without a second integration. Connect the [Skorvia MCP](/docs/mcp) once, or read how the [Search Console MCP](/google-search-console-mcp) fits together, then run the workflow with this prompt.

```text
Using the Skorvia MCP, pull Search Console query data for
