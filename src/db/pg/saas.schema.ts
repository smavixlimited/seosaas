...
export const competitorTrackedAds = pgTable(
  "competitor_tracked_ads",
  {
    ...
    isWinningAd: boolean("is_winning_ad").notNull().default(false),
    isAiOpportunity: boolean("is_ai_opportunity").notNull().default(false),
...
export const waitlistUsers = pgTable(
  ...
  (table) => [
    index("pg_waitlist_email_idx").on(table.email),
    index("pg_waitlist_status_idx").on(table.status),
    index("pg_waitlist_created_at_idx").on(table.createdAt),
  ],
);
...
export const backlinkProspects = pgTable(
  ...
  (table) => [
    index("pg_backlink_prospects_project_idx").on(table.projectId),
    index("pg_backlink_prospects_domain_idx").on(table.prospectDomain),
