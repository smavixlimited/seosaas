...
import type { CompetitorStrategyTeardown, AttackPlayItem } from "@/services/competitor-strategy.service";
...
export function CompetitorStrategyTab({
  ...
  const regenerateMutation = useMutation({
    ...
    onSuccess: (data) => {
      queryClient.setQueryData(["competitorStrategy", projectId, domain, locationCode], data);
    ...
    return (
      <div className="p-12 text-center space-y-4">
        <div className="mx-auto h-12 w-12 rounded-2xl bg-primary/10 flex items-center justify-center text-primary animate-pulse">
          <Icon icon="solar:shield-check-bold-duotone" className="h-6 w-6 animate-spin" />
        </div>
        <div className="space-y-1">
          ...
          <p className="text-xs text-slate-500 max-w-md mx-auto">
            Analyzing organic keyword clusters, commercial search intent, content moats, and conversion friction to construct your 5-Pillar Battle Plan.
          </p>
        </div>
      </div>
    );
  ...
  const { positioning, funnelAngles, contentMoat, vulnerabilities, attackPlaybook, rawMetricsSummary } = report;
    ...
    <div className="p-6 space-y-8 animate-in fade-in duration-150">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-200 dark:border-slate-700">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-400 font-mono">Target: {domain}</span>
          </div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400">
            Synthesized intelligence identifying market positioning, 80/20 content moats, page-2 vulnerabilities, and actionable attack plays.
          </p>
        </div>
        ...
        >
          <Icon icon="solar:refresh-circle-bold" className={`h-4 w-4 text-primary ${regenerateMutation.isPending ? "animate-spin" : ""}`} />
          <span>{regenerateMutation.isPending ? "Analyzing..." : "Refresh Teardown"}</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/60 space-y-0.5">
          <span className="text-[11px] text-slate-400 font-medium">Est. Monthly Organic Traffic</span>
        ...
        <div className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/60 space-y-0.5">
          <span className="text-[11px] text-slate-400 font-medium">Ranking Organic Keywords</span>
          <h4 className="text-base font-bold text-slate-800 dark:text-slate-100 font-mono">
            {rawMetricsSummary.organicKeywords.toLocaleString()}
          </h4>
        </div>
        <div className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/60 space-y-0.5">
          <span className="text-[11px] text-slate-400 font-medium">Total Backlink Equity</span>
          <h4 className="text-base font-bold text-slate-800 dark:text-slate-100 font-mono">
            {rawMetricsSummary.backlinks.toLocaleString()}
          </h4>
        </div>
        <div className="p-3.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/60 space-y-0.5">
          <span className="text-[11px] text-slate-400 font-medium">Referring Domains</span>
          <h4 className="text-base font-bold text-slate-800 dark:text-slate-100 font-mono">
        ...
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-2xs space-y-4">
          <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-700 pb-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400">How the competitor frames value and targets buyers.</p>
            </div>
          </div>

          <div className="space-y-3 text-xs">
            <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Primary Value Promise</span>
            ...
            <div className="grid grid-cols-2 gap-2 text-[11px]">
              <div className="p-2.5 rounded-lg border border-slate-100 dark:border-slate-700">
                ...
                <span className="font-bold text-slate-700 dark:text-slate-200">{positioning.targetAudience}</span>
              </div>
              <div className="p-2.5 rounded-lg border border-slate-100 dark:border-slate-700">
                <span className="text-slate-400 block text-[10px]">Market Stance</span>
                <span className="font-bold text-slate-700 dark:text-slate-200">{positioning.marketStance}</span>
              </div>
            </div>

            <div>
              <span className="text-[11px] font-bold text-slate-600 dark:text-slate-300 block mb-1.5">Core Messaging Pillars:</span>
              <div className="space-y-1">
                {positioning.messagingStrengths.map((s, idx) => (
                  <div key={idx} className="flex items-center gap-2 text-[11px] text-slate-600 dark:text-slate-300">
                    <Icon icon="solar:check-circle-bold-duotone" className="h-3.5 w-3.5 text-emerald-500 shrink-0" />
                    <span>{s}</span>
                  </div>
                ))}
              </div>
        ...
        <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-2xs space-y-4">
          <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-700 pb-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400">Value drivers, offer structures, and buyer friction points.</p>
            </div>
          </div>

          <div className="space-y-3 text-xs">
            <div className="p-3 rounded-lg bg-slate-50 dark:bg-slate-700/30 border border-slate-100 dark:border-slate-700">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Estimated Funnel Model</span>
              <p className="font-semibold text-slate-800 dark:text-slate-100 mt-0.5">
            ...
            <div>
              <span className="text-[11px] font-bold text-rose-600 dark:text-rose-400 flex items-center gap-1 mb-1">
                ...
                <span>Pricing &amp; Sales Friction (Where They Are Vulnerable)</span>
              </span>
              <div className="space-y-1">
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-2xs space-y-4">
        <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-700 pb-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400">The 3 core content clusters generating the vast majority of their organic visibility.</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-xs">
          {contentMoat.topThemes.map((theme, idx) => (
            <div key={idx} className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/60 dark:bg-slate-800/40 space-y-2">
              <div className="flex items-center justify-between">
                <span className="font-bold text-slate-800 dark:text-slate-100 text-xs truncate">{theme.theme}</span>
              ...
              <div className="space-y-1">
                <span className="text-[10px] text-slate-400 block">Intent Profile:</span>
                <span className="text-[11px] font-semibold text-slate-700 dark:text-slate-200 block">{theme.intentDistribution}</span>
              </div>

              <div>
                <span className="text-[10px] text-slate-400 block mb-1">Core Cluster Terms:</span>
                <div className="flex flex-wrap gap-1">
                  {theme.coreKeywords.map((kw, kwIdx) => (
                    <span key={kwIdx} className="px-1.5 py-0.5 rounded text-[10px] font-mono bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-200">
                      {kw}
                    </span>
                  ))}
                </div>
      ...
      <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-2xs space-y-4">
        <div className="flex items-center gap-2 border-b border-slate-100 dark:border-slate-700 pb-3">
          ...
          <div>
            <h4 className="text-xs font-bold text-slate-800 dark:text-slate-100">
              Pillar 4: Strategic Gaps &amp; Striking-Distance Keywords (Ranks #11–#30)
            </h4>
            <p className="text-[10px] text-slate-400">High-commercial queries where the rival is weak and page 1 is ripe for capture.</p>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700/60 text-slate-700 dark:text-slate-200">
              {vulnerabilities.strikingDistanceKeywords.map((k, idx) => (
                <tr key={idx} className="hover:bg-slate-50/60 dark:hover:bg-slate-700/30">
                  ...
                  <td className="px-3 py-2.5 font-mono">{k.searchVolume.toLocaleString()} /mo</td>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs pt-2">
          <div className="p-3.5 rounded-lg border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/40 space-y-1.5">
            <span className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1 text-[11px]">
              <Icon icon="solar:document-text-bold-duotone" className="h-4 w-4 text-amber-500" />
              <span>Identified Content Gaps</span>
            </span>
            <div className="space-y-1">
              {vulnerabilities.contentWeaknesses.map((w, idx) => (
                <p key={idx} className="text-[11px] text-slate-500 dark:text-slate-400 flex items-start gap-1.5">
          ...
          <div className="p-3.5 rounded-lg border border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/40 space-y-1.5">
            <span className="font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1 text-[11px]">
              <Icon icon="solar:code-bold-duotone" className="h-4 w-4 text-rose-500" />
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h4 className="text-sm font-bold text-slate-800 dark:text-slate-100 flex items-center gap-2">
                <span>Pillar 5: The Attack Playbook (1-Click SAM AI Execution)</span>
              </h4>
              <p className="text-xs text-slate-500 dark:text-slate-400">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${
                    ...
                    {play.priority === "QUICK_WIN" ? "QUICK WIN" : `${play.priority} PRIORITY`}
                  </span>

                  <span className="text-[10px] text-slate-400 font-medium">Effort: {play.estimatedEffort}</span>
                </div>
                ...
                <div className="space-y-1 text-xs">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Action Steps:</span>
