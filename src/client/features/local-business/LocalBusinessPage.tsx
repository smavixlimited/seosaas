...
export function LocalBusinessPage({ projectId }: LocalBusinessPageProps) {
  ...
  const [gridKeyword, setGridKeyword] = React.useState("corporate office near me");
  ...
  const [selectedReview, setSelectedReview] = React.useState<LocalReviewItem | null>(null);
  ...
  const [oauthStep, setOauthStep] = React.useState<"login" | "detecting" | "select">("login");
  ...
  const fallbackData: LocalBusinessData = React.useMemo(() => ({
    profile: {
      ...
      citations: [
        { directory: "Google Business Profile", url: "https://maps.google.com", name: "Smavix Limited", address: "KM 17 Lekki - Epe Expressway, Lagos, 106104, NG", phone: "+234 805 716 2832", status: "mismatch" },
        { directory: "Apple Maps", url: "https://maps.apple.com", name: "Smavix Limited", address: "KM 17 Lekki - Epe Expressway, Lagos", phone: "+234 805 716 2832", status: "missing" },
        { directory: "Bing Places", url: "https://bingplaces.com", name: "Smavix Limited", address: "KM 17 Lekki - Epe Expressway, Lagos", phone: "+234 805 716 2832", status: "missing" },
        { directory: "Facebook", url: "https://facebook.com", name: "N & D Giftery Private Limited", address: "KM 17 Lekki - Epe Expressway, Lagos", phone: "+234 805 716 2832", status: "mismatch" },
      ],
      reviews: [
        { id: "rev_1", author: "Babatunde Adebayo", rating: 5, relativeTime: "3 days ago", text: "Fast service and very reliable team at Lekki. Highly recommended for corporate consulting!", response: "Thank you so much Babatunde!", responsePublishedAt: new Date(Date.now() - 86400000).toISOString(), sentiment: "positive" },
      ],
      auditHighlights: [
  ...
  const localQuery = useQuery<LocalBusinessData>({
    ...
    queryFn: () => getLocalBusinessDashboard({ data: {} }) as Promise<LocalBusinessData>,
  ...
  const activeLocation = locations.find((l) => l.id === (selectedLocationId || data.activeLocationId)) || locations[0];
  ...
  const detectedProfilesQuery = useQuery<DetectedGoogleBusinessProfile[]>({
    ...
    queryFn: () =>
      detectGoogleProfilesServerFn({
        data: { userEmail: session?.user?.email || "smartwareinnovation@gmail.com" },
      }),
  ...
  const campaignKitQuery = useQuery<ReviewCampaignKit>({
    queryKey: ["reviewCampaignKit", activeLocation?.businessName || profile?.businessName],
    queryFn: () =>
      getReviewCampaignKitServerFn({
        data: {
          businessName: activeLocation?.businessName || profile?.businessName || "My Brand",
  ...
  const scanGridMutation = useMutation({
    mutationFn: (vars: { keyword: string; size: "3x3" | "5x5"; lat: number; lng: number }) =>
      runLocalGeoGridScan({
        data: {
    ...
    onSuccess: (newGrid: any) => {
      void queryClient.invalidateQueries({ queryKey: ["localBusinessDashboard", projectId] });
      toast.success(`Geo-grid scan complete for "${newGrid.keyword}"! Avg Rank: #${newGrid.averageRank}`);
    },
    onError: (err: Error) => {
  ...
  const connectDetectedMutation = useMutation({
    ...
    onSuccess: (updatedData) => {
      queryClient.setQueryData(["localBusinessDashboard", projectId], updatedData);
      setIsOAuthModalOpen(false);
      setOauthStep("login");
      toast.success(`Connected Google Business Profile: "${updatedData.profile.businessName}"!`);
    },
    onError: (err: Error) => {
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 animate-in fade-in duration-150">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-4 border-b border-base-300">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-base-content/50 font-mono">Google Business Profile &amp; Listings</span>
          </div>
          <h1 className="text-2xl font-black tracking-tight text-base-content mt-1">
            Local Business
          </h1>
          <p className="text-xs text-base-content/60 leading-relaxed">
            From Google to AI search—show up everywhere local customers search, with optimized Google Business Profile, listings, reviews, and rankings.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          >
            <Icon icon="solar:refresh-circle-bold" className={`h-4 w-4 ${localQuery.isFetching ? "animate-spin" : ""}`} />
          </button>
        </div>
      </div>
      ...
      <div className="tabs tabs-boxed bg-base-200/60 p-1 rounded-2xl w-fit flex-wrap">
        <button
          ...
          className={`tab tab-sm font-bold rounded-xl gap-1.5 transition-all ${
            activeTab === "overview" ? "tab-active !bg-primary !text-white shadow-sm" : "text-base-content/70"
          }`}
        >
        ...
        <button
          ...
          className={`tab tab-sm font-bold rounded-xl gap-1.5 transition-all ${
            activeTab === "grid" ? "tab-active !bg-primary !text-white shadow-sm" : "text-base-content/70"
          }`}
        >
        ...
        <button
          ...
          className={`tab tab-sm font-bold rounded-xl gap-1.5 transition-all ${
            activeTab === "reviews" ? "tab-active !bg-primary !text-white shadow-sm" : "text-base-content/70"
          }`}
        >
        ...
        <button
          ...
          className={`tab tab-sm font-bold rounded-xl gap-1.5 transition-all ${
            activeTab === "campaign" ? "tab-active !bg-primary !text-white shadow-sm" : "text-base-content/70"
          }`}
        >
        ...
        <button
          ...
          className={`tab tab-sm font-bold rounded-xl gap-1.5 transition-all ${
            activeTab === "nap" ? "tab-active !bg-primary !text-white shadow-sm" : "text-base-content/70"
          }`}
        >
          ...
          <div className="rounded-3xl border border-base-300 bg-base-100 p-6 md:p-8 shadow-sm space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-base-200">
              <div className="flex items-center gap-2">
                ...
                <span className="badge badge-sm badge-success text-white font-bold">Connected via Google OAuth</span>
              </div>

              <div className="flex items-center gap-3 text-xs">
                <button
                  type="button"
                  onClick={() => copyToClipboard(window.location.href, "Report link copied!")}
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <Icon icon="solar:map-point-bold" className="h-5 w-5 text-primary shrink-0" />
              ...
              <p className="text-xs text-base-content/70 pl-7">
                {activeLocation?.streetAddress || profile?.streetAddress}, {activeLocation?.city || profile?.city}, {activeLocation?.postalCode || profile?.postalCode}, {activeLocation?.countryCode || profile?.countryCode}, {activeLocation?.phoneNumber || profile?.phoneNumber}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-2">
              ...
              <div className="p-5 rounded-2xl bg-base-200/40 border border-base-300 space-y-2">
                <div className="text-xs font-bold text-base-content/60">Online presence</div>
                <div className="text-[11px] text-base-content/50 font-medium">Overall assessment</div>
              ...
              <div className="p-5 rounded-2xl bg-base-200/40 border border-base-300 space-y-2">
                <div className="text-xs font-bold text-base-content/60">Listings to fix</div>
                <div className="text-[11px] text-base-content/50 font-medium">Upon subscription</div>
                <div className="text-2xl font-black text-base-content font-mono pt-1">
                  <span className="text-error">{profile?.listingsToFixCount || 30}</span>
                  <span className="text-base-content/40 text-lg font-normal"> / {profile?.totalListingsCount || 33}</span>
                </div>
              </div>

              {/* Card 3: Average Star Rating */}
              <div className="p-5 rounded-2xl bg-base-200/40 border border-base-300 space-y-2">
                <div className="text-xs font-bold text-base-content/60">Average star rating</div>
                <div className="text-[11px] text-base-content/50 font-medium">Customers love high ratings</div>
                <div className="text-lg font-black text-base-content flex items-center gap-1.5 pt-2">
                  {profile?.totalReviews && profile.totalReviews > 0 ? (
                    <>
                      <span className="text-amber-500">⭐ {profile.averageRating}</span>
                      <span className="text-xs text-base-content/50 font-normal">({profile.totalReviews} reviews)</span>
                    </>
                  ) : (
                    <>
                      <span className="text-warning">⚠️</span>
                      <span className="text-base-content/80 text-base font-bold">No reviews</span>
                    </>
                  )}
                </div>
              </div>
          ...
          <div className="rounded-3xl border border-base-300 bg-base-100 p-6 shadow-sm space-y-4">
            <div>
              ...
              <p className="text-xs text-base-content/60">
                Directory-by-directory verification showing missing citations, mismatched addresses, or incorrect business phone numbers.
              </p>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="table table-sm w-full">
                ...
                <tbody className="divide-y divide-base-200 text-xs">
                  {profile?.directoriesCoverage?.map((cov: DirectoryCoverageItem, idx: number) => {
                    const isMissing = cov.status === "Not Present" || cov.status === "No Address";
                    ...
                    return (
                      <tr key={idx} className="hover:bg-base-200/40 transition-colors">
                        <td className="font-bold text-base-content flex items-center gap-2 py-3">
                          <Icon icon="solar:shop-bold" className="h-4 w-4 text-primary shrink-0" />
                        ...
                        <td>
                          {isMatched ? (
                            ...
                            <span className={`font-bold ${isMissing ? "text-error" : "text-amber-600"}`}>
                              {cov.status}
                            </span>
                          )}
                        </td>
        ...
        <div className="space-y-6">
          <div className="rounded-3xl border border-base-300 bg-base-100 p-6 shadow-sm space-y-4">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <div>
                <h3 className="text-base font-black text-base-content">
                  Local Map Pack Geo-Grid: <span className="text-primary font-mono">{grid?.keyword}</span>
                </h3>
                <p className="text-xs text-base-content/60">
                  Simulating local Google Maps queries across GPS coordinate pins around {activeLocation?.locationName || "your Brand"}.
                </p>
              </div>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                <button
                  ...
                  className={`btn btn-xs rounded-xl font-bold gap-1 ${
                    showTimelineCompare ? "btn-primary text-white" : "btn-outline"
                  }`}
                >
                  <Icon icon="solar:history-bold" className="h-3.5 w-3.5" />
                  <span>{showTimelineCompare ? "Timeline View Active" : "Compare History"}</span>
                </button>
                ...
                <button
                  ...
                  className="btn btn-sm btn-primary rounded-xl font-bold text-white shadow-md shadow-primary/20 gap-1.5"
                >
                  <Icon icon="solar:radar-2-bold" className={`h-4 w-4 ${scanGridMutation.isPending ? "animate-spin" : ""}`} />
                  <span>{scanGridMutation.isPending ? "Scanning Pins..." : "Run Grid Scan"}</span>
                </button>
              </div>
            </div>
            ...
            <div className="p-6 rounded-2xl bg-base-200/50 border border-base-300 flex flex-col items-center justify-center space-y-6">
              <div className="flex items-center gap-4 text-xs font-bold flex-wrap justify-center">
                <span className="flex items-center gap-1 text-emerald-600">
                  <span className="size-3 rounded-full bg-emerald-500" /> Rank 1–3 (Map Pack Winner)
                </span>
                <span className="flex items-center gap-1 text-amber-500">
                  <span className="size-3 rounded-full bg-amber-500" /> Rank 4–10 (Page 1)
                </span>
                <span className="flex items-center gap-1 text-rose-500">
                  <span className="size-3 rounded-full bg-rose-500" /> Rank 11+ (Low Visibility)
                </span>
              </div>
              ...
              >
                {grid?.points.map((pt: LocalGridPoint, idx: number) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-base sm:text-xl font-black font-mono">#{pt.rank}</span>
                      {showTimelineCompare && pt.rankDelta !== undefined && pt.rankDelta !== 0 ? (
                        <span className={`text-[10px] font-black font-mono ${pt.rankDelta > 0 ? "text-emerald-600" : "text-rose-500"}`}>
                          {pt.rankDelta > 0 ? `▲ +${pt.rankDelta}` : `▼ ${pt.rankDelta}`}
                        </span>
                      ) : (
                        <span className="text-[9px] text-base-content/50 font-mono">{pt.distanceKm}km</span>
                      )}
                    </div>
                  );
                })}
      ...
      {activeTab === "reviews" && (
        <div className="space-y-6">
          <div className="rounded-3xl border border-base-300 bg-base-100 p-6 shadow-sm space-y-4">
            <div>
              <h3 className="text-base font-black text-base-content">Customer Reviews &amp; Direct 1-Click AI Publishing</h3>
              <p className="text-xs text-base-content/60">Draft tone-aware AI replies with Skorvia and publish them straight to Google Maps.</p>
            </div>

            <div className="space-y-3">
              {profile?.reviews.map((rev: LocalReviewItem) => (
                <div key={rev.id} className="p-4 rounded-2xl bg-base-200/40 border border-base-300 space-y-3">
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-sm text-base-content">{rev.author}</span>
                      <span className="text-xs text-amber-500 font-bold">{"⭐".repeat(rev.rating)}</span>
                      <span className="text-xs text-base-content/40 font-mono">{rev.relativeTime}</span>
                    </div>

                    <button
                      type="button"
                      onClick={() => handleOpenAiReply(rev, activeLocation?.businessName || profile.businessName)}
                    ...
                    >
                      ...
                      <span>{rev.response ? "Edit / Re-Draft Reply" : "Draft AI Reply"}</span>
                    </button>
                  </div>
                    ...
                    <div className="p-3 rounded-xl bg-primary/5 border border-primary/20 text-xs text-base-content/80 space-y-1">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-[10px] uppercase text-primary tracking-wider flex items-center gap-1">
                          <Icon icon="solar:check-circle-bold" className="h-3 w-3 text-emerald-500" />
                          <span>Owner Reply (Published Live to Google)</span>
                        </span>
                        {rev.responsePublishedAt && (
                          <span className="text-[10px] text-base-content/40 font-mono">
                            {new Date(rev.responsePublishedAt).toLocaleDateString()}
                          </span>
                        )}
                      </div>
                      <p>{rev.response}</p>
        ...
        <div className="space-y-6">
          <div className="rounded-3xl border border-base-300 bg-base-100 p-6 shadow-sm space-y-6">
            <div>
              ...
              <p className="text-xs text-base-content/60">
                Generate high-converting short review links, printable in-store QR code flyers, and ready-to-send SMS/Email review request templates.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-5 rounded-2xl bg-base-200/50 border border-base-300 space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h4 className="font-bold text-sm text-base-content">Short Google Review URL</h4>
                    <p className="text-[11px] text-base-content/60">Send this 1-click link to recent customers.</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="text"
                    readOnly
                    value={campaignKit?.shortReviewUrl || activeLocation?.reviewLink || "https://g.page/r/yourbrand/review"}
                    className="input input-bordered input-sm w-full rounded-xl text-xs font-mono bg-base-100"
                  />
                  <button
                    type="button"
                    onClick={() => copyToClipboard(campaignKit?.shortReviewUrl || activeLocation?.reviewLink || "", "Google Review URL copied!")}
              ...
              <div className="p-5 rounded-2xl bg-base-200/50 border border-base-300 flex items-center justify-between gap-4">
                <div className="space-y-1">
                  <h4 className="font-bold text-sm text-base-content">In-Store Printable QR Code</h4>
            ...
            <div className="space-y-4">
              <h4 className="text-sm font-black text-base-content">High-Converting Outreach Templates</h4>
              
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {campaignKit?.smsTemplates.map((sms) => (
                  <div key={sms.id} className="p-4 rounded-2xl bg-base-200/40 border border-base-300 space-y-2 flex flex-col justify-between">
                    <div className="space-y-1">
                      <div className="flex items-center justify-between">
                        ...
                        <span className="badge badge-xs badge-ghost font-mono">SMS</span>
                      </div>
                    ...
                    <button
                      type="button"
                      onClick={() => copyToClipboard(sms.message, "SMS template copied!")}
      ...
      {activeTab === "nap" && (
        <div className="space-y-6">
          <div className="rounded-3xl border border-base-300 bg-base-100 p-6 shadow-sm space-y-4">
            <div>
              <h3 className="text-base font-black text-base-content">Name, Address &amp; Phone (NAP) Consistency</h3>
              <p className="text-xs text-base-content/60">
                Auditing business citations across major global business directories to prevent local ranking penalties for {activeLocation?.locationName}.
              </p>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="table table-sm w-full">
                ...
                <tbody>
                  {profile?.citations.map((cit: LocalCitationItem, idx: number) => (
                    <tr key={idx} className="border-b border-base-200 text-xs font-medium">
                      <td className="font-bold text-base-content">{cit.directory}</td>
                      <td>{cit.name}</td>
                      <td className="text-base-content/70">{cit.address}</td>
                      <td className="font-mono text-base-content/70">{cit.phone}</td>
                      <td>
          ...
          <div className="rounded-3xl bg-base-100 border border-base-300 p-6 max-w-lg w-full shadow-2xl space-y-5 animate-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-base-300 pb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="font-black text-base text-base-content">Connect Google Business Profile</h3>
                  <p className="text-[11px] text-base-content/60">Direct OAuth synchronization (No manual entry required)</p>
                </div>
              </div>
              <button
                type="button"
            ...
            {oauthStep === "login" && (
              <div className="space-y-4 py-2 text-center">
                ...
                <div className="space-y-1">
                  <h4 className="text-sm font-black text-base-content">Authorize Google Account</h4>
                  <p className="text-xs text-base-content/60 max-w-sm mx-auto">
                    Sign in with the Google Account that manages your Google Business Profile. Skorvia will auto-detect your locations.
                  </p>
                </div>
                ...
                <div className="p-3 rounded-2xl bg-base-200/50 border border-base-300 text-xs text-base-content/70 flex items-center justify-center gap-2 font-mono">
                  <Icon icon="solar:user-bold" className="h-4 w-4 text-primary" />
                  <span>{userEmail}</span>
                </div>
                ...
                >
                  <Icon icon="logos:google-icon" className="h-5 w-5 bg-white rounded-full p-0.5" />
            ...
            {oauthStep === "detecting" && (
              <div className="py-8 text-center space-y-4">
                <Icon icon="solar:refresh-circle-bold" className="h-10 w-10 text-primary animate-spin mx-auto" />
                <div className="space-y-1">
                  <h4 className="text-sm font-black text-base-content">Connecting to Google Business Profile API...</h4>
                  <p className="text-xs text-base-content/60">Scanning verified business locations managed by {userEmail}...</p>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-base-content/50 font-mono text-[11px]">{userEmail}</span>
                  </div>
                  <h4 className="text-sm font-black text-base-content">
                    Select a Google Business Profile to Connect:
                  </h4>
                  <p className="text-xs text-base-content/60">
                    We found <strong>{detectedProfilesQuery.data?.length || 3} locations</strong> managed by your account. Select which one to link:
                  </p>
                </div>
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="space-y-0.5">
                          <div className="font-black text-sm text-base-content flex items-center gap-1.5">
                            <Icon icon="solar:shop-2-bold" className="h-4 w-4 text-primary" />
                            <span>{item.businessName}</span>
                          </div>
                          <p className="text-xs text-base-content/70">{item.streetAddress}, {item.city}, {item.countryCode}</p>
                          <p className="text-[11px] text-base-content/50 font-mono">{item.phoneNumber}</p>
                        </div>

                        <span className={`badge badge-sm font-bold ${item.onlineAssessment === "Poor" ? "badge-warning" : "badge-success text-white"}`}>
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-base-300/60">
                        ...
                        <button
                          ...
                          onClick={() => connectDetectedMutation.mutate(item.id)}
                          className="btn btn-xs btn-primary rounded-xl font-bold text-white shadow-xs gap-1 px-4"
                        >
                          <Icon icon="solar:link-circle-bold" className="h-3.5 w-3.5" />
                          <span>{connectDetectedMutation.isPending ? "Connecting..." : "Connect This Profile"}</span>
                        </button>
                      </div>
                    </div>
                  ))}
          ...
          <div className="rounded-3xl bg-base-100 border border-base-300 p-6 max-w-lg w-full shadow-2xl space-y-4 animate-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-base-300 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-black text-base text-base-content">Skorvia AI Review Responder</h3>
              </div>
              <button
                type="button"
            ...
            <div className="p-3 rounded-2xl bg-base-200/50 border border-base-300 space-y-1">
              <div className="flex items-center justify-between text-xs font-bold text-base-content">
                ...
                <span className="text-amber-500">{"⭐".repeat(selectedReview.rating)}</span>
              </div>
              <p className="text-xs text-base-content/70 italic">&ldquo;{selectedReview.text}&rdquo;</p>
            </div>

            <div className="space-y-2">
              <label className="text-xs font-bold text-base-content">Generated Response Draft</label>
              {isDrafting ? (
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  <span>{publishReplyMutation.isPending ? "Publishing..." : "Publish to Google"}</span>
                </button>
              </div>
            </div>
          </div>
