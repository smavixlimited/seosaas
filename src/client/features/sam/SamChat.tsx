...
export function SamChat({
  ...
  if (activeSessionId) {
    ...
    return (
      <div className="flex h-full min-h-0 flex-col">
        ...
        <div className="flex items-center justify-between gap-3 border-b border-base-300 px-5 py-3 bg-base-100/50 backdrop-blur-sm">
          <div className="flex items-center gap-3 min-w-0">
            <span className="truncate text-sm font-black text-base-content">
              {activeTitle ?? "Skorvia AI Copilot"}
            </span>
          </div>
          <Link
            to="/p/$projectId/settings/context"
            params={{ projectId }}
            className="flex shrink-0 items-center gap-1.5 text-xs font-bold text-base-content/70 transition-colors hover:text-primary rounded-xl px-2.5 py-1 hover:bg-base-200"
          >
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-4 p-6 text-center">
      ...
      <button
        type="button"
        className="btn btn-primary btn-sm rounded-xl px-5 font-bold text-white shadow-md shadow-primary/20 gap-1.5"
      ...
      >
        ...
        <span>Start New Growth Chat</span>
      </button>
    </div>
  );
}
