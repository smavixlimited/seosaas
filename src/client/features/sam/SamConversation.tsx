...
const SUGGESTIONS = [
  "Audit my site and prioritize 3 high-impact ranking fixes",
  "Find easy-win keywords with high search volume and low KD",
  "Decode competitor content strategy and find keyword gaps",
  "Audit my Google Business Profile and local Map Pack rankings",
  "Analyze landing page CRO and ad spend leakage risk",
  "Check our AI / AEO visibility across ChatGPT, Perplexity & Claude",
  "Analyze backlink gap against top 3 competitors",
  "How is my Google Search Console traffic trending this month?",
  "Generate 10 blog post outlines matching our target cluster",
];
    ...
    <div className="relative flex min-w-0 flex-1 flex-col">
      ...
      >
        <div className="mx-auto max-w-2xl space-y-6">
          {messages.length === 0 ? (
            <div className="space-y-3 text-sm text-base-content/85">
            ...
            <div className="flex flex-wrap gap-2">
              {SUGGESTIONS.map((question) => (
                <button
                  key={question}
                  type="button"
                  className="rounded-xl border border-base-300 bg-base-100 px-3.5 py-2 text-xs font-semibold text-base-content/80 transition-all hover:border-primary hover:text-primary hover:bg-base-200/50 shadow-2xs text-left"
      ...
      <div className="flex-shrink-0 border-t border-base-300 px-5 py-3 bg-base-100/60 backdrop-blur-sm">
        <div className="mx-auto w-full max-w-2xl">
          <ChatComposer
            busy={isBusy}
            onSend={sendText}
            placeholder="Ask Skorvia to diagnose, decode, or scale your Brand…"
          />
        </div>
      </div>
    </div>
