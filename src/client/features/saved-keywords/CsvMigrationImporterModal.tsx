...
export function parseMigrationCsv(csvContent: string): {
  ...
  if (
    headers.includes("keyword difficulty") ||
    headers.includes("difficulty (%)")
  ) {
    ...
    headers.includes("difficulty") &&
    headers.includes("opportunity")
  ) {
    detectedSource = "Moz";
  } else if (
    headers.includes("search volume") &&
    headers.includes("competition")
  ) {
  ...
  const kwIndex = headers.findIndex((h) =>
    ["keyword", "keyword phrase", "query", "search term", "keywords"].includes(
      h,
    ),
  );
  ...
  const kdIndex = headers.findIndex((h) =>
    ["kd", "keyword difficulty", "difficulty", "difficulty (%)"].includes(h),
  );
  const cpcIndex = headers.findIndex((h) =>
    ["cpc", "cpc (usd)", "cost per click"].includes(h),
  );
  const intentIndex = headers.findIndex((h) =>
    ["intent", "search intent"].includes(h),
  );
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const vol =
      volIndex >= 0
        ? Number(rawCols[volIndex]?.replace(/[^0-9.]/g, ""))
        : undefined;
    const kd =
      kdIndex >= 0
        ? Number(rawCols[kdIndex]?.replace(/[^0-9.]/g, ""))
        : undefined;
    const cpc =
      cpcIndex >= 0
        ? Number(rawCols[cpcIndex]?.replace(/[^0-9.]/g, ""))
        : undefined;
    const intent =
      intentIndex >= 0
        ? rawCols[intentIndex]?.replace(/^["']|["']$/g, "").trim()
        : undefined;
...
export function CsvMigrationImporterModal({
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      ...
      toast.success(
        `Successfully imported ${parsed.keywords.length} keywords from ${parsed.detectedSource}!`,
      );
      onClose();
    } catch {
      ...
      <div className="modal-box max-w-xl rounded-3xl border border-base-300 p-6 space-y-6 shadow-2xl">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-extrabold text-base text-base-content">
                Universal CSV Migration Importer
              </h3>
              <p className="text-xs text-base-content/60">
                Import keyword lists directly from Ahrefs, Semrush, Moz, or
                custom CSVs.
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-base-200/50 border border-base-300 space-y-3">
            ...
            <div className="max-h-40 overflow-y-auto space-y-1.5 pr-1 font-mono text-[11px]">
              {parsed.keywords.slice(0, 5).map((kw, idx) => (
                <div
                  key={idx}
                  className="p-2 rounded-xl bg-base-100 border border-base-300/80 flex items-center justify-between"
                >
                  <span className="font-bold text-base-content">
                    {kw.keyword}
                  </span>
              ...
              {parsed.keywords.length > 5 && (
                <div className="text-[10px] text-center text-base-content/50 pt-1">
                  + {parsed.keywords.length - 5} more keywords ready to
                  import...
                </div>
              )}
            </div>
        ...
        <div className="flex items-center justify-end gap-3 pt-2">
          ...
          >
            {isImporting
              ? "Importing Keywords..."
              : `Import ${parsed?.keywords.length || 0} Keywords &rarr;`}
          </button>
        </div>
      </div>
    </div>
