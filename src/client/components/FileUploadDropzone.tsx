  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div>
          <label className="text-xs font-bold text-base-content block">{label}</label>
          {description && <p className="text-[11px] text-base-content/60">{description}</p>}
        </div>
        {currentUrl && onRemove && (
          <button
            type="button"
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div
              className={`rounded-xl border border-base-300 bg-base-100 p-2 flex items-center justify-center overflow-hidden shrink-0 ${
                aspectRatio === "square" ? "w-16 h-16" : aspectRatio === "wide" ? "w-28 h-16" : "w-20 h-16"
              }`}
            >
              {!imageError ? (
                <img
            ...
            <div className="min-w-0">
              <span className="text-xs font-bold text-base-content block truncate">{currentUrl.split("/").pop() || label}</span>
              <span className={`text-[10px] font-bold flex items-center gap-1 mt-0.5 ${imageError ? "text-amber-500" : "text-emerald-600"}`}>
                <Icon icon={imageError ? "solar:info-circle-bold" : "solar:check-circle-bold"} className="h-3 w-3" />
                <span>{imageError ? "Asset linked (preview pending)" : "Uploaded & Active"}</span>
              </span>
            </div>
          </div>
        ...
        >
          {isUploading ? (
            <div className="space-y-2 w-full max-w-xs flex flex-col items-center">
              ...
              <div className="text-xs font-bold text-base-content">Uploading to R2... {uploadProgress}%</div>
              <progress className="progress progress-primary w-full" value={uploadProgress || 20} max="100" />
            </div>
          ) : (
            <>
              <div className="w-10 h-10 rounded-2xl bg-primary/10 text-primary flex items-center justify-center shadow-xs">
                <Icon icon="solar:cloud-upload-bold-duotone" className="h-5 w-5" />
              </div>
              <div>
